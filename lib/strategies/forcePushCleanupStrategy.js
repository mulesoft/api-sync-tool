...
module.exports = function (apiPlatformService, BPromise, logger, messages,
    workspaceRepository) {
  return {
    cleanup: cleanup
  };
  ...
  function cleanup() {
    ...
    function deleteFiles(apiFilesMetadata) {
      var remoteFiles = _.reject(apiFilesMetadata, 'isDirectory');
      rootRamlFile = _.find(remoteFiles, 'id', rootRamlFileId);
      remoteFiles = _.reject(remoteFiles, 'id', rootRamlFileId);
      logger.info(messages.deletingAllFilesMessage());
      return BPromise.each(remoteFiles, function (remoteFile) {
