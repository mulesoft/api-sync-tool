...
module.exports = function (apiPlatformService, BPromise,
    errors, localService, logger, messages, workspaceRepository) {
  return {
    push: push
  };
  ...
  function push() {
      ...
      function createAPIDirectory(newDirectoryPath) {
        ...
        return apiPlatformService.createAPIDirectory(workspace.bizGroup.id,
          ...
          .then(function (newRemoteDirectory) {
            ...
            workspace.directories.push(
              _.pick(newRemoteDirectory, 'path', 'audit'));
          });
      }
    ...
    .then(function () {
      ...
      return pushNewFiles(workspace, status.added, remoteFiles)
        ...
        .then(function () {
          ...
          return pushDeletedDirectories(workspace, status.deletedDirectories,
            remoteFiles);
        })
        .then(function () {
          result.deletedDirectories = status.deletedDirectories;
          return result;
        });
    })
    ...
    .catch(function (err) {
      if (workspace) {
        // Only update workspace if error happened after obtaining it.
        return workspaceRepository.update(workspace)
          .then(reject);
      } else {
        return reject();
  ...
  function pushDeletedDirectories(workspace, deletedDirectories, remoteFiles) {
